...
**We send no API key.** `JITEN_API_KEY` is not set on the project, and the functions only attach `X-Api-Key` when it is — so every call goes out anonymously and Jiten's anonymous figures (~300 req/min general, **~10 req/min on the vocabulary endpoint**) are simply our figures. Getting a key would move us to a higher bucket and is worth doing: **one episode sync is already ~10 vocabulary requests, so a single sync very nearly exhausts the anonymous budget for that whole minute.**

**What this achieves and what it doesn't.** It stops one caller monopolising the allowance. It does **not** bound the total — Jiten meters our egress, not our users, and two people syncing an episode in the same minute exceed ~10 req/min however well each is behaving individually. A global ceiling is the only mechanism that fixes that. Until then the practical mitigations are the ones already in place: sync is idempotent (each episode fetched once ever), and a Jiten 429 surfaces as a clean error rather than corrupting anything.
