...
**UI behavior:** `#/vocab`'s home screen is the textbook chapter path (`TextbookHomeScreen`) once a textbook is active. Everything else — picking a different list to drill, and browsing any list's actual words — lives in one modal, `WordExplorerModal`, with two internal steps: **Picker** (titled "Drill any list" — a `Select` of sources plus a `DataList` multi-select of the chosen source's sublists, reached via the "Free drill" button) and **Words** (titled "View words" — the word list itself, kanji breakdown, sentences, grouped under each list's own `SectionHeader` — what used to be the standalone `GlanceScreen` full page). A Back button on Words returns to Picker rather than closing the modal, so exploring one list, going back, and picking another is one continuous flow. Three entry points converge on this same modal: "Free drill" opens at Picker; a chapter row's "View words" and a Dictionary entry's "Vocab Drill match" link (`?chapter=<listKey>`) both open straight to Words, pre-seeded with that chapter — and neither needs an active textbook, since the modal doesn't depend on `showTextbookScreen`. There is no bare source-picker page any more — the old full-page `HomeScreen`/`SubListTile` grid was deleted once nothing in the app's own navigation reached `#/vocab` without a textbook already chosen (the dashboard's card gates that behind `TextbookPicker`); a visit to `#/vocab` with no textbook and no `?chapter=` link redirects home instead. That redirect is a one-time decision (`arrivedViaChapterLink`, captured at mount): a chapter-link visit is permanently opted out of it, so closing the sheet after peeking at a word list — with or without an active textbook — never itself triggers a surprise navigation to the dashboard.
