...
| Data List | Built, migrated in Anime Vocab | `EpisodeVocabBrowser`, `EpisodeList`, `TrackedAnimeSection`, `EpisodeDrill`'s `DoneScreen`, `MediaSearch`'s list results (all migrated) · `VocabPage`'s `DoneScreen` (×2), `WordImportPanel`, `DictionaryPage`'s `EntryRow`, `DictionaryEntryPage`'s `DeckRow`, `GrammarMapModule`'s prereq rows (not yet migrated) |
