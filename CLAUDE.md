...
| Data List | Built, migrated everywhere | `EpisodeVocabBrowser`, `EpisodeList`, `EpisodeDrill`'s `DoneScreen`, `MediaSearch`'s list results, `DictionaryPage`'s `EntryRow`, `DictionaryEntryPage`'s `DeckRow`, `VocabPage`'s `DoneScreen` + Preview groups, `VocabSrsBrowsePage`'s card list, `WordImportPanel`'s review table (all migrated). Gained `navigate.href`, `rowState`, `selection.bulkHeader: { selectFirst }`, and per-column `placeholder` for editable cells along the way. `TrackedAnimeSection` moved off this onto `Pinned Shelf` — see settled decision #20 |
...
| Pinned Shelf | Built — composes Feed Card (`image` mode) + Button (`ghost-muted` remove overlay) | `TrackedAnimeSection`'s "Currently studying" list, moved off `Data List` |
...
20. **A curated pinned set is `Pinned Shelf`, not a re-generalized `Data List`.** `TrackedAnimeSection`'s "Currently studying" had the same unbounded-growth problem `Data List` always has — no cap, no scroll, so it pushed the entire search screen down as more shows got tracked. The fix is a bounded, horizontally-scrolling row of cover tiles (composes `Feed Card`'s `image` mode + `Button`'s `ghost-muted` as a remove overlay) that stays one row tall no matter how many items are pinned. This does **not** generalize to News's unbuilt "read articles" history: a pinned set is small, user-curated, explicit add/remove; a reading history is unbounded, auto-populated, and nobody removes from it. The two only share the bounded-scroll *shape* — if/when a "Continue reading" surface gets built for News, give it its own component sharing that shape, not a mode flag on this one (the same reasoning that killed the "Info Row" idea for `Data List`).
