...
| `src/components/TextbookPicker.jsx` | "Change textbook" — `Modal` + the split browser (`TextbookBrowser`, also exported for the lab): book list beside the selected book's cover, description and buy links. On mobile (`stacked`) the detail becomes a `position: sticky` block above the list — **not** a nested scroller: the sheet is max-height-driven, so a percentage-height child silently falls back to auto and pushes the confirm button below the fold. Measured budget on a 375×667 phone: 532px sheet, 261px pinned detail, ~210px (≈5 rows) of list visible; 393×852 gives ~8 rows |
