import { FONT, TRACKING, TEXT_MUTED, FS_BASE, LANTERN_ON, LANTERN_OFF, LANTERN_SIZES } from '../data/theme.js'
...
export default function CenteredLoadingMessage({ text }) {
  return (
    <div style={{ textAlign: 'center', padding: '48px 0', color: TEXT_MUTED, fontFamily: FONT, fontSize: FS_BASE, letterSpacing: TRACKING }}>
      <span style={{ position: 'relative', display: 'inline-block', width: LANTERN_SIZES.nav, height: LANTERN_SIZES.nav, marginBottom: 10 }}>
        <img src={LANTERN_ON} alt="" width={LANTERN_SIZES.nav} height={LANTERN_SIZES.nav} className="lantern-flicker-on" style={{ position: 'absolute', inset: 0, display: 'block', imageRendering: 'pixelated' }} />
        <img src={LANTERN_OFF} alt="" width={LANTERN_SIZES.nav} height={LANTERN_SIZES.nav} className="lantern-flicker-off" style={{ position: 'absolute', inset: 0, display: 'block', imageRendering: 'pixelated' }} />
      </span>
      <div>{text}</div>
    </div>
  )
