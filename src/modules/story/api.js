...
export async function generateStory({ learnerContext, mode, basedOn, format, length, questionCount }) {
  const data = await invoke('story-generate', { learnerContext, mode, basedOn, format, length, questionCount })
...
// → { pass, feedback }
export function gradeAnswer({ question, correctAnswer, acceptableVariations, userAnswer }) {
  return invoke('story-grade', { question, correctAnswer, acceptableVariations, userAnswer })
}
